  ...
  const connections = await Promise.all(
    config.connections.map(async ({ chain_a, client_a, chain_b, client_b }) => {
      ...
      return {
        a: {
          chain: chainA,
          denom: chainA.fees?.fee_tokens?.[0]?.denom ?? 'uthiolx',
          wallet: walletA,
          address: addressA,
          client: client_a,
          stargate: stargateA,
        ...
        b: {
          chain: chainB,
          denom: chainB.fees?.fee_tokens?.[0]?.denom ?? 'uscrt',
          wallet: walletB,
          address: addressB,
          client: client_b,
          stargate: stargateB,
...
// TODO: Add logic to handle automating upgrading any expired clients 
// try {
//   // a. get all clients
//   const existing_clients = await spawnPromise('rly', [
//     'q',
//     'clients-expiration',
//     `${a.chain.chain_name}-${b.chain.chain_name}`,
//   ])
//   console.log(existing_clients)
//   // a.1 filter by expired
//   // todo()!
//   // b. create new clients 
//   const new_clients = await spawnPromise('rly', [
//     'tx',
//     'clients',
//     `${a.chain.chain_name}-${b.chain.chain_name}`, 
//     '--override'
...
//   // c. propose to upgrade clients 
//   const propose_update = await spawnPromise('terpd', [
//     'tx',
//     'submit-legacy-proposal',
//     'update-client', 
//     '07-tendermint-', // todo: subject client-id
//     '07-tendermint-', // todo: new client-id
//   ])
//   console.log(propose_update)
